...
namespace LabApplication.User
{
    public partial class ServiceDetails : System.Web.UI.Page
    {
        ...
        private void showServiceDetails()
        {
            ...
            string query = @"Select * from LServices where ServiceId = @id";
            ...
            DataList1.DataBind();
            serviceTitle = dt.Rows[0]["Headding"].ToString();
        ...
        protected void DataList1_ItemCommand(object source, DataListCommandEventArgs e)
        {
            if (e.CommandName == "ApplyService")
            {
                if (Session["user"] != null)
                {
                    ...
                    {
                        con = new SqlConnection(str);
                        string query = @"Insert into Appointments values(@ServiceId, @UserId)";
                        cmd = new SqlCommand(query, con);
        ...
        {
            con = new SqlConnection(str);
            string query = @"Select * from Appointments where UserId=@UserId and ServiceId=@ServiceId";
            cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@UserId", Session["userId"]);
            cmd.Parameters.AddWithValue("@ServiceId", Request.QueryString["id"]);
            sda = new SqlDataAdapter(cmd);
